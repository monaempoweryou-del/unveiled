...
// Inventory is tracked per product only. Driver-carried stock and multi-location
// warehouses are deliberately outside this MVP, so a movement needs no location.
...
export async function deliverOrder(id, { final_amount, payment_status, payment_method, note }){
  ...
  // Delivery turns the reservation into a sale: stock leaves once, and the
  // payment method recorded by the Owner has no effect on inventory.
